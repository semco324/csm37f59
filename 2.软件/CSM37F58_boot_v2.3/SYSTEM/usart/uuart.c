...
void User_UART0_Init(void)
{
    UART_InitTypeDef UART_InitStruct;
	  ...
	  GPIO_SetMode(GPIO_PA3,GPIO_Mode_OUTPUT);
...
//  System_SetGPIOReuse(Reuse_PA2,UART_RXD); //设置PA10为 rx
    System_SetGPIOReuse(Reuse_PA3,UART_TXD); //设置PA9为 tx
    
    UART_Init(&UART_InitStruct); //初始化串口并使能
}

//			UART_SendByte(RESVal->F1_Z1_RES_H);	
